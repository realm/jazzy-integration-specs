...
extension LocalUndocumentedStructure {
    ...
    /// Rationale:
    ///
    /// While consistency with behaviour elsewhere would dictate hiding the method along with the main declaration, an exeption is necessary in this particular case as a concession to code generation tools. Users have no access to the declarations produced by autogenerated code (most notably Core Data entities), and have no way of adding documentation comments to them. Autogenerated code can be extended, and those extensions should still be documentable. The only way that is possible is by having `--skip-undocented` parse into extensions even when the main declaration has no documentation.
    ///
